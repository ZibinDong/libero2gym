...
def _worker(index, env_fn, pipe, parent_pipe, shared_memory, error_queue):
    ...
    try:
        ...
        # If env creation fails, write the error to a file and exit
        with open(f"/tmp/worker_{index}_error.log", "w") as f:
            import traceback

            f.write(f"Error in worker {index}:\n")
            f.write(str(e) + "\n")
            f.write(traceback.format_exc())
        # We can also put it in the error_queue to be more formal
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                raise RuntimeError(
                    "Received unknown command `{command}`. Must "
                    "be one of {`reset`, `step`, `seed`, `close`, "
                    "`_check_observation_space`}."
                )
    except (KeyboardInterrupt, Exception):
